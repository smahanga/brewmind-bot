...
// ════════════════════════════════════════════════════════════════════
const QUICK_STARTS = [
  { id: "safety_test", title: "🛡️ Safety Stress Test", subtitle: "Can the health bot be tricked?", persona: "bad_actor", bot: "health_assistant", turns: 4, color: "#E74C3C" },
  { id: "empathy_test", title: "💛 Empathy Challenge", subtitle: "How does support handle fury?", persona: "angry_customer", bot: "techflow_support", turns: 4, color: "#F39C12" },
  { id: "accessibility_test", title: "🌍 Accessibility Test", subtitle: "Can the tutor help an ESL student?", persona: "esl_speaker", bot: "tutor_bot", turns: 4, color: "#27AE60" },
  { id: "patience_test", title: "👵 Patience Test", subtitle: "Finance bot vs. confused grandma", persona: "confused_grandma", bot: "finance_advisor", turns: 4, color: "#8E44AD" }
];
...
export default function App() {
  ...
  const resetAll = () => {
    ...
    setView("welcome"); setSelectedPersona(null); setSelectedBot("techflow_support");
  ...
  // auto-run for quick-start
  useEffect(() => {
    if (view === "setup_then_run" && selectedPersona && targetPrompt) runSimulation();
  }, [view, selectedPersona, targetPrompt, runSimulation]);
  ...
  const renderWelcome = () => (
    <div style={S.container}>
      <div style={{ textAlign: "center", padding: "48px 0 36px" }}>
        ...
        <p style={{ color: "#555", fontSize: 13, maxWidth: 420, margin: "0 auto" }}>
          Pick a scenario below to watch it live, or build your own test.
        </p>
      </div>
      ...
      <div style={S.sectionTitle}>⚡ Quick Start — Pick a Scenario</div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10, marginBottom: 24 }}>
        {QUICK_STARTS.map(q => {
          const persona = PERSONAS.find(p => p.id === q.persona);
          const bot = TARGET_BOTS.find(b => b.id === q.bot);
          return (
            <div key={q.id} onClick={() => {
              setSelectedPersona(q.persona); setSelectedBot(q.bot);
              setTargetPrompt(bot.prompt); setMaxTurns(q.turns);
              setTimeout(() => {}, 50);
              setView("setup_then_run");
            }}
              style={{ ...S.card, cursor: "pointer", padding: "16px 18px", borderLeft: `4px solid ${q.color}`, display: "flex", alignItems: "center", gap: 14 }}
              onMouseEnter={e => e.currentTarget.style.background = "#222228"}
              onMouseLeave={e => e.currentTarget.style.background = "#1A1A1F"}>
              <div style={{ fontSize: 32 }}>{persona.icon}</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 2 }}>{q.title}</div>
                <div style={{ fontSize: 12, color: "#888" }}>{q.subtitle}</div>
                <div style={{ marginTop: 6, display: "flex", gap: 6 }}>
                  <Pill color={persona.color}>{persona.name}</Pill>
                  <Pill color="#666">vs</Pill>
                  <Pill color="#2471A3">{bot.name}</Pill>
                </div>
              </div>
      ...
      <div style={{ textAlign: "center" }}>
        <button style={{ ...S.btn(false), border: "1px solid #444", color: "#ccc", padding: "12px 32px" }}
          onClick={() => setView("setup")}>🔧&nbsp; Build Your Own Test</button>
      </div>
  ...
  const renderSetup = () => (
    <div style={S.container}>
      ...
      {selectedBot === "external_api" ? (
        ...
        <div style={S.card}>
          <div style={S.sectionTitle}>System Prompt {selectedBot !== "custom" && "(editable)"}</div>
      ...
      <div style={{ textAlign: "center", marginTop: 8 }}>
        {(() => {
          ...
          return (<>
            ...
            {!canRun && <div style={{ fontSize: 12, color: "#555", marginTop: 6 }}>{!selectedPersona ? "Select a persona" : "Configure the target bot"}</div>}
          </>);
        })()}
      </div>
    </div>
    ...
    return (
      <div style={S.container}>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "center", marginTop: 10 }}>
          <button style={{ ...S.btn(false), padding: "12px 28px" }} onClick={() => setView("setup")}>🔧 Modify & Re-run</button>
  ...
  return (
    <div style={S.root}>
      ...
      <div style={S.header}>
        ...
        {view !== "welcome" && <button onClick={resetAll} style={{ padding: "6px 14px", borderRadius: 8, border: "1px solid #333", background: "transparent", color: "#888", fontFamily: "'Space Grotesk'", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>↻ Reset</button>}
      </div>
      {view === "welcome" && renderWelcome()}
      {(view === "setup" || view === "setup_then_run") && renderSetup()}
